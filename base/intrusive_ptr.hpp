...
// NOTE: sp_convertible is different from base::is_convertible
// (in base/type_traits.h) that it converts pointers only. Using
// base::is_convertible results in ctor/dtor issues.
...
template<typename T>
struct hash<base::intrusive_ptr<T> > {
    std::size_t operator()(const base::intrusive_ptr<T>& p) const {
        return hash<T*>()(p.get());
    }
};
#elif defined(COMPILER_MSVC)
template<typename T>
inline size_t hash_value(const base::intrusive_ptr<T>& sp) {
