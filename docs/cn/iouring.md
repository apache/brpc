...
| `iouring_max_inflight_writes` | `64` | 单个 Socket 上允许同时在途（已提交未完成）的 WRITE/WRITE_FIXED SQE 数量上限；达到上限后 `IsWritable()` 返回 `false`，写入退化为同步 fd 写，可按硬件/场景调优 |
...
| `iouring_mem_pool_initial_mb` | `8` | 初始注册内存大小（MiB），单个 region 内部按 `UIO_MAXIOV`（1024 block）自动截断，见下方说明 |
| `iouring_mem_pool_increase_mb` | `8` | 内存池扩容步长（MiB），同样受 `UIO_MAXIOV` 截断 |
...
>
> **`UIO_MAXIOV` 截断**：单次 buffer 注册调用最多注册 `UIO_MAXIOV`（内核常量，通常为 1024）个 iovec，与 `readv`/`writev` 共用同一限制。若 `--iouring_mem_pool_initial_mb` / `--iouring_mem_pool_increase_mb` 换算出的 block 数超过这个上限，`AddRegion` 会自动把该次 region 截断到 `1024 * --iouring_iobuf_block_size` 字节并打印 WARNING，不会导致注册失败；超出的部分会在后续通过 `--iouring_mem_pool_max_regions` 允许的额外扩容触发。
>
> **多 region 注册机制**：每个 Poller 的 ring 在创建时会立即调用 `io_uring_register_buffers_sparse()`，一次性预留 `--iouring_mem_pool_max_regions * UIO_MAXIOV` 个空 buf_index 槽位（对应内存池理论最大容量）。之后每次 `AddRegion()` 扩容都通过 `io_uring_register_buffers_update_tag()` 把新 region 的 iovec 原地写入自己的槽位区间，不影响任何已存在 region 的 buf_index——因此扩容期间旧 region 分配出去、正在使用中的 buffer 始终保持有效，不存在失效或错位的窗口期。该机制依赖内核对 sparse buffer 注册的支持（Linux ≥ 5.19）；若目标内核过旧，进程会在启用 `--iouring_register_buffers` 时于启动阶段直接失败退出（`LOG(FATAL)`），而不是带着不完整的注册状态继续运行。
...
Allocate()
  └─ 1. TLS 快路径：从当前线程的 tls_free_ 链表头取块（无锁，O(1)）
       ...
       2. 全局桶路径：对 thread_id % num_free_buckets_ 选桶，加锁后
          ...
          桶也空 ↓
       3. 扩容路径：调用 AddRegion() 分配新 slab 并向所有已注册 ring 调
          io_uring_register_buffers_update_tag() 原地注册新槽位区间，
          计入 iouring_mem_pool_pool_grow_count
...
- `iouring_mem_pool_pool_grow_count` 在运行期间持续增长，说明初始内存不足，可增大 `--iouring_mem_pool_initial_mb` 或 `--iouring_mem_pool_increase_mb`（注意单个 region 仍受 `UIO_MAXIOV` 截断，见上方说明；需要更大总容量时应同时调大 `--iouring_mem_pool_max_regions`）。
