...
-lb_warmup_curve (default 1.0) shapes the ramp: the traffic share is max(0.1, progress^lb_warmup_curve) where progress rises linearly from 0 to 1 over the window. Values above 1 keep a new server colder for longer, values below 1 ramp more aggressively.
