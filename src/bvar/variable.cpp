...
namespace bvar
{
    ...
    std::string read_command_name()
    {
        ...
        if (cmdline.empty())
        {
            return std::string(); // Return empty string if the file is empty.
        }

        // Extract the first null-terminated string (the command name).
        std::string::size_type pos = cmdline.find('\0');
        std::string command_name = (pos != std::string::npos) ? cmdline.substr(0, pos) : cmdline;

        // Normalize the command name if it starts with '(' and ends with ')'.
        std::string s;
        if (command_name.size() >= 2UL && command_name.front() == '(' && command_name.back() == ')')
        {
            to_underscored_name(&s, command_name.substr(1, command_name.size() - 2UL));
        }
        else
        {
    ...
    {
        if (format != "common" && format != "prometheus")
        {
