...
private:
  ...
  void *allocate_aligned_in_other_blocks(size_t n);
...
inline void *Arena::allocate_aligned(size_t n) {
  if (_cur_block != NULL) {
    char *ptr = _cur_block->data + _cur_block->alloc_size;
    const size_t align = sizeof(void *);
    // Align the pointer to a multiple of sizeof(void*)
    size_t off = (uintptr_t)ptr & (align - 1);
    if (off != 0) {
      off = align - off;
    }
    if (_cur_block->left_space() >= n + off) {
      _cur_block->alloc_size += n + off;
      return ptr + off;
    }
  }
  return allocate_aligned_in_other_blocks(n);
}
