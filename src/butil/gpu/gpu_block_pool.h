...
public:
    // Returns the process-wide leaky singleton instance. Created on first
    // access via butil::Singleton and intentionally never destroyed (GPU/RDMA
    // resources outlive normal static destructors).
    static BlockPoolAllocators* GetInstance();
...
private:
    // Allow DefaultSingletonTraits::New() to construct the instance.
    friend struct DefaultSingletonTraits<BlockPoolAllocators>;
