...
BlockPoolAllocators* BlockPoolAllocators::GetInstance() {
    return ::Singleton<BlockPoolAllocators,
                       LeakySingletonTraits<BlockPoolAllocators>>::get();
}
...
bool InitGPUBlockPool(int gpu_id, ibv_pd* pd) {
    BlockPoolAllocators::GetInstance()->init(gpu_id, pd);
    return true;
}
