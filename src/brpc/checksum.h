...
struct ChecksumHandler {
    ...
    void (*Compute)(const butil::IOBuf& buf, Controller* cntl);

    // verify buf checksum
    // Rerturn true on success, false otherwise
    bool (*Verify)(const butil::IOBuf& buf, const Controller& cntl);
...
void ComputeDataChecksum(const butil::IOBuf& data, Controller* cntl,
                         ChecksumType checksum_type);

// Verify `data' checksum Returns true on success, false otherwise
bool VerifyDataChecksum(const butil::IOBuf& data, const Controller& cntl,
                        ChecksumType checksum_type);
