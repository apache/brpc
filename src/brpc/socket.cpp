...
private:
	  // 'sid' is the lightest load socket id. 
    // 'load' is the pending rpc count when 'sid' selected as lightest sid.
    struct SidWithLoad {
        SocketId sid;
        uint32_t load; 
    };

    SidWithLoad BuildSidWithLoad(const SocketId sid, const uint32_t load) {
        SidWithLoad s;
        s.sid = sid;
        s.load = load;
        return s;
    ...
    inline void UpdateLightestSocketIfNeeded(const SidWithLoad s);

    butil::atomic<size_t> _num_created;
    butil::EndPoint _remote_side;
    butil::atomic<SidWithLoad> _lightest;
...
void SocketPool::Describe(std::ostream& os) const {
    os << "  numfree=" << _numfree.load(butil::memory_order_relaxed)
...
SocketMultiple::SocketMultiple(const SocketOptions& opt)
    ...
    SidWithLoad lsid = BuildSidWithLoad(INVALID_SOCKET_ID, 0);
    _lightest.store(lsid);
}
...
int SocketMultiple::GetSocket(SocketUniquePtr* ptr_out) {
    ...
    SidWithLoad lsid = _lightest.load(butil::memory_order_acquire);
    if (Socket::Address(lsid.sid, &lptr) == 0) {
        uint32_t load = lptr->_rpc_count.load(butil::memory_order_relaxed);
        if (load <= threshold || (load - lsid.load) <= threshold) {
    ...
    SocketId sid;
    SocketId min_sid = INVALID_SOCKET_ID;
    ...
    for (size_t i = 0; i != _multiple.size(); ++i) {
        ...
        if (addressed == 0) {
            ...
            if (load == 0) {
                SidWithLoad lsid = BuildSidWithLoad(sid, 0);
        ...
        if (sid == INVALID_SOCKET_ID) {
            ...
            if (init >= 0 && Socket::Address(new_id, ptr_out) == 0) {
                ...
                if (init == 0) {
                    SidWithLoad lsid = BuildSidWithLoad(new_id, 0);
                    _lightest.store(lsid, butil::memory_order_relaxed); 
                }
                return 0;
            }
    ...
    if (min_sid != INVALID_SOCKET_ID && Socket::Address(min_sid, ptr_out) == 0) {
        SidWithLoad s = BuildSidWithLoad(
...
void SocketMultiple::ReturnSocket(Socket* sock) {
    ...
    SidWithLoad lsid = _lightest.load(butil::memory_order_relaxed);
    SidWithLoad s = BuildSidWithLoad(
        sock->id(), sock->_rpc_count.load(butil::memory_order_relaxed));
    if (s.sid == lsid.sid) {
        if (s.load < lsid.load) {
...
int SocketMultiple::InitSocket(const size_t index, SocketId* sid) {
    ...
    if (count < _multiple.size()) {
        psid = reinterpret_cast<butil::atomic<SocketId>*>(&_multiple[count]);
        if (!ptr) {
        ...
        if (ptr) {
            psid = reinterpret_cast<butil::atomic<SocketId>*>(&_multiple[index]);
        }
    }
    if (ptr) {
...
inline void SocketMultiple::UpdateLightestSocketIfNeeded(const SidWithLoad s) {
    SidWithLoad lsid = _lightest.load(butil::memory_order_acquire);
    do {
        SocketUniquePtr ptr;
        if (Socket::Address(lsid.sid, &ptr) == 0 && lsid.sid != s.sid) {
            if (s.load + FLAGS_threshold_for_switch_multiple_connection >= 
...
void SocketMultiple::Describe(std::ostream& os) const {
    SocketUniquePtr ptr;
    int load = -1;
    SidWithLoad lsid = _lightest.load(butil::memory_order_relaxed);
    if (Socket::Address(lsid.sid, &ptr) == 0) {
        load = ptr->_rpc_count.load(butil::memory_order_relaxed);
    }
    os << "  numcreated=" << _num_created.load(butil::memory_order_relaxed)
       << "\n  LightestCurrentLoad=" << load << " LightestSelectedLoad=" << lsid.load;
}
...
bool Socket::GetSocketGroupCount(int* num) {
    SharedPart* sp = GetSharedPart();
    if (sp == nullptr) {
        return false;
    }
