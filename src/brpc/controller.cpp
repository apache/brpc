...
Controller::Call::Call(Controller::Call *rhs)
    : nretry(rhs->nretry), need_feedback(rhs->need_feedback), peer_id(rhs->peer_id), begin_time_us(rhs->begin_time_us), sending_sock(rhs->sending_sock.release()), stream_user_data(rhs->stream_user_data)
{
...
END_OF_RPC:
    if (new_bthread)
    {
        ...
        bthread_t bt;
        bthread_attr_t attr = (FLAGS_usercode_in_pthread ? BTHREAD_ATTR_PTHREAD : BTHREAD_ATTR_NORMAL);
...
void Controller::IssueRPC(int64_t start_realtime_us)
{
    ...
    if (_connection_type == CONNECTION_TYPE_SINGLE ||
        _stream_creator != NULL)
    { // let user decides the sending_socket
                                   ...
                                   const std::string &error_text)
{
    ...
    if (!cntl->is_used_by_rpc())
    {
        ...
        // the RPC.
        cntl->SetFailed(error_code, "Cancel call_id=%" PRId64 " before CallMethod()", id.value);
...
Controller::CreateProgressiveAttachment(StopStyle stop_style)
{
    ...
    {
        LOG(ERROR) << "Only http supports ProgressiveAttachment now" << _request_protocol;
        return NULL;
    }
    if (_current_call.sending_sock == NULL)
    {
