...
private:
#if BRPC_WITH_IOURING
    // Creates (if necessary) and allocates io_uring resources for _socket.
    // No-op if io_uring is unavailable, resources are already allocated, or
    // _socket does not yet have a valid, connected fd -- see OnFdReady() for
    // why this must be deferred for lazily-connected Sockets (fd==-1 right
    // after Init()).
    void MaybeSetupIouring();

    // The optional io_uring endpoint for this socket. NULL when io_uring is
    // disabled globally, the socket has no valid fd yet, or per-socket
    // resource allocation failed; in that case every method falls back to
    // the synchronous fd path below.
    iouring::IouringEndpoint* _iouring_ep = nullptr;

    // Copied from SocketOptions::is_listen_socket in Init(). MaybeSetupIouring()
    // (called both from Init() and, for lazily-ready fds, from OnFdReady()
    // afterwards -- at which point the original SocketOptions is long gone)
    // uses this to refuse to set up an IouringEndpoint over a passively-
    // listening fd: io_uring's read path would eagerly SubmitRead() against
    // it, and the kernel correctly rejects a read on a non-connected/listening
    // socket with -ENOTCONN, which looks exactly like (and used to be
    // misdiagnosed as) a stalled/lost write.
    bool _is_listen_socket = false;
#endif  // BRPC_WITH_IOURING
};
} // namespace brpc
