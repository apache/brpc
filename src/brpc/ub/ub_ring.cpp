...
RETURN_CODE UBRing::UbrTrxClose() {
    ...
    if (((UbrEventQMsg *)_trx->ubrTx.localTxEventQ.addr)->flag == UBR_STATE_CONNECTED) {
    ...
    while (((UbrEventQMsg *)_trx->ubrRx.localRxEventQ.addr)->flag != UBR_STATE_CLOSED) {
        UbrSetSleepTask(UBR_TASK_CLOSE);
        if (HasTimedOut(startTime, disconnectTimeout) != HLC_OK) {
    ...
    _trx->ubrRx.trxState = UBR_STATE_CLOSED;
    RETURN_CODE rc;
    if (UNLIKELY((rc = ClearTrxResource(_trx, startTime, UBR_SEND_CLOSE)) != HLC_OK)) {
...
void* UBRing::UbrTrxCloseCallback(void* args) {
    ...
    do {
        ...
        if (UNLIKELY(ClearTrxResource(trx, startTime, UBR_CALL_BACK_CLOSE, 1) != HLC_OK)) {
...
void *UBRing::UbrAsynClearCallback(void *args)
{
    ...
    if (UNLIKELY(ShmRemoteFree(&trx->remoteShm) != HLC_OK)) {
...
int UBRing::UbrTrxRecv(void *buf, uint32_t bufLen)
{
    ...
    if (UNLIKELY((rc = CheckTrxRecvParam(_trx, buf, bufLen)) != HLC_OK)) {
...
{
    RETURN_CODE rc = HLC_OK;
    if (UNLIKELY((rc = CheckTrxRecvParam(_trx, dest, bufLen)) != HLC_OK)) {
        return (rc == UBR_NOT_CONNECTED) ? 0 : rc;
    }
...
RETURN_CODE UBRing::UbrTrxMapLocalShm(SHM *localShm)
{
    ...
    if (UNLIKELY(localShm == NULL)) {
        LOG(ERROR) << "Trx map Shared memory failed, localShm is null.";
        return HLC_ERR;
    }
...
RETURN_CODE UBRing::UbrTrxMapRemoteShm(SHM *remoteShm)
{
    ...
    if (UNLIKELY(remoteShm == NULL)) {
        LOG(ERROR) << "Trx map Shared memory failed, remoteShm is null.";
        return HLC_ERR;
    }
...
RETURN_CODE UBRing::UbrServerTrxInit(SHM *localShm, SHM *remoteShm)
{
    ...
    rc = UBRingManager::GetHlcDealMsgMaxCnt(_trx->ubrRx.capacity, &_trx->ubrRx.dealMsgMaxCnt);
    if (UNLIKELY(rc != HLC_OK)) {
...
int UBRing::UbrAllocateServerShm(SHM* remote_trx_shm, SHM* local_trx_shm) {
    ...
    if (UNLIKELY((UBRingManager::AcquireUbrTrxFromMgr(ubrTrxPtr)) != HLC_OK)) {
...
RETURN_CODE UBRing::UbrMapRemoteShmAddTimer(SHM *localTrxShm, const char *localName)
{
    ...
    if (HasTimedOut(startTime, timeout) != HLC_OK) {
...
RETURN_CODE UBRing::ApplyAndMapLocalShm(SHM *localTrxShm, const char *localName)
{
    ...
    if (UNLIKELY(rc != HLC_OK)) {
        LOG(ERROR) << "Trx apply local shared memory failed, local shm name=" << localTrxShm->name;
    ...
    rc = UBRingManager::GetHlcDealMsgMaxCnt(_trx->ubrRx.capacity, &_trx->ubrRx.dealMsgMaxCnt);
    if (rc != HLC_OK) {
        LOG(ERROR) << "Get hlc deal msg max cnt, local shm name=" << localTrxShm->name;
        ShmLocalFree(localTrxShm);
        UBRingManager::ReleaseUbrTrxFromMgr(_trx);
        return rc;
    }
...
RETURN_CODE UBRing::UbrClearResourceCheck(UbrTrx *trx, uint64_t startTime, UbrCloseType closeType)
{
    ...
    while (ATOMIC_LOAD(trx->closeCnt) == 1 && localTxEventQ->flag == UBR_STATE_CLOSING) {
        if (HasTimedOut(startTime, FLAGS_ub_disconnect_timeout) != HLC_OK) {
