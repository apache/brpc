...
// consumed (1, 3, 4, or 9).
//
// 0xFB is the protocol's nullptr marker (a nullptr column value in a result
// row), NOT an ordinary integer: when |buf| begins with 0xFB the value is
// nullptr, *out is set to 0, *is_null (when non-nullptr) is set to true, and 1
// (the single byte consumed) is returned.  For every non-nullptr result
...
// the caller does not need to distinguish nullptr from 0.
...
// number of bytes consumed.  A leading 0xFB encodes the protocol nullptr
// value: when present *out_value is cleared, *is_null (when non-nullptr) is
// set to true, and 1 (the marker byte) is returned.  For a non-nullptr
