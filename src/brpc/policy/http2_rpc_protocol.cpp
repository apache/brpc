...
H2Context::H2Context(Socket *socket, const Server *server)
    : _socket(socket), _remote_window_left(H2Settings::DEFAULT_INITIAL_WINDOW_SIZE), _conn_state(H2_CONNECTION_UNINITIALIZED), _last_received_stream_id(-1), _last_sent_stream_id(1), _goaway_stream_id(-1), _deferred_window_update(0)
{
    ...
    butil::IOBufBytesIterator &it, H2FrameHead *frame_head)
{
    ...
    const uint32_t length = ((uint32_t)length_buf[0] << 16) | ((uint32_t)length_buf[1] << 8) | length_buf[2];
    ...
    butil::IOBufBytesIterator &it, const H2FrameHead &frame_head)
{
    ...
    H2StreamContext *sctx = NULL;
    if (is_server_side() &&
        frame_head.stream_id > _last_received_stream_id)
    { // new stream
    ...
    butil::IOBufBytesIterator &it, const H2FrameHead &frame_head)
{
    ...
    const int64_t window_diff =
        static_cast<int64_t>(_remote_settings.stream_window_size) - old_stream_window_size;
    if (window_diff)
    {
...
#endif
      ,
      _stream_id(0), _stream_ended(false), _remote_window_left(0), _deferred_window_update(0), _correlation_id(INVALID_BTHREAD_ID.value)
{
...
H2UnsentRequest *H2UnsentRequest::New(Controller *c)
{
    ...
    const size_t maxsize = h.HeaderCount() + 4 + (size_t)need_content_type + (size_t)need_accept + (size_t)need_user_agent + (size_t)need_authorization;
    ...
    if (scheme->empty())
    {
        scheme = (c->is_ssl() ? &common->H2_SCHEME_HTTPS : &common->H2_SCHEME_HTTP);
    }
...
H2UnsentResponse::H2UnsentResponse(Controller *c, int stream_id, bool is_grpc)
    : _size(0), _stream_id(stream_id), _http_response(c->release_http_response()), _is_grpc(is_grpc)
{
