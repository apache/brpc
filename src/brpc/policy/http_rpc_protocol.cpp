...
CommonStrings::CommonStrings()
    : ACCEPT("accept"), DEFAULT_ACCEPT("*/*"), USER_AGENT("user-agent"), DEFAULT_USER_AGENT("brpc/1.0 curl/7.0"), CONTENT_TYPE("content-type"), CONTENT_TYPE_TEXT("text/plain"), CONTENT_TYPE_JSON("application/json"), CONTENT_TYPE_PROTO("application/proto"), ERROR_CODE("x-bd-error-code"), AUTHORIZATION("authorization"), ACCEPT_ENCODING("accept-encoding"), CONTENT_ENCODING("content-encoding"), GZIP("gzip"), CONNECTION("connection"), KEEP_ALIVE("keep-alive"), CLOSE("close"), LOG_ID("log-id"), DEFAULT_METHOD("default_method"), NO_METHOD("no_method"), H2_SCHEME(":scheme"), H2_SCHEME_HTTP("http"), H2_SCHEME_HTTPS("https"), H2_AUTHORITY(":authority"), H2_PATH(":path"), H2_STATUS(":status"), STATUS_200("200"), H2_METHOD(":method"), METHOD_GET("GET"), METHOD_POST("POST"), TE("te"), TRAILERS("trailers"), GRPC_ENCODING("grpc-encoding"), GRPC_ACCEPT_ENCODING("grpc-accept-encoding"), GRPC_ACCEPT_ENCODING_VALUE("identity,gzip"), GRPC_STATUS("grpc-status"), GRPC_MESSAGE("grpc-message"), GRPC_TIMEOUT("grpc-timeout")
{
}
                          ...
                          const google::protobuf::Message *pbreq)
{
    ...
    if (method != NULL)
    {
        ...
        std::string path;
        path.reserve(2 + method->service()->full_name().size() + method->name().size());
        path.push_back('/');
...
public:
    ...
    HttpResponseSender(HttpResponseSender &&s)
        : _cntl(std::move(s._cntl)), _req(std::move(s._req)), _res(std::move(s._res)), _method_status(std::move(s._method_status)), _received_us(s._received_us), _h2_stream_id(s._h2_stream_id)
    {
    }
    ~HttpResponseSender();
...
HttpResponseSender::~HttpResponseSender()
{
    ...
    else if (cntl->has_progressive_writer())
    {
        ...
        if (!cntl->response_attachment().empty() && !is_grpc)
        {
                            ...
                            std::string *unresolved_path)
{
    ...
    const Server::ServiceProperty *const sp =
        (full_service_name ? wrapper.FindServicePropertyByFullName(service_name) : wrapper.FindServicePropertyByName(service_name));
    if (NULL == sp)
    {
...
bool VerifyHttpRequest(const InputMessageBase *msg)
{
    ...
    const std::string *authorization = http_request->header().GetHeader("Authorization");
...
void ProcessHttpRequest(InputMessageBase *msg)
{
    ...
    if (is_http2)
    {
        ...
        cntl->grpc_stream_id = h2_sctx->stream_id();
    }
        ...
        else
        {
            ...
            if (is_http2)
            {
                if (is_grpc_ct)
                {
                    ...
                    // add stream setting
                    H2StreamContext *h2_sctx = static_cast<H2StreamContext *>(msg);
                    brpc::StreamSettings *setting = new brpc::StreamSettings();
                    setting->set_stream_id(h2_sctx->stream_id());
                    setting->set_need_feedback(true);
                    setting->set_writable(true);

                    accessor.set_remote_stream_settings(setting);
                }
            }
            else
            {
