...
inline SocketOptions::SocketOptions()
    ...
    , connect_abstime(nullptr)
    , user(nullptr)
    , on_edge_triggered_events(nullptr)
    ...
    , conn(nullptr)
    , app_connect(nullptr)
    , initial_parsing_context(nullptr)
...
inline Destroyable* Socket::release_parsing_context() {
    return _parsing_context.exchange(nullptr, butil::memory_order_acquire);
}
