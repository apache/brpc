...
// UIO_MAXIOV is only conditionally defined by glibc's <sys/uio.h> (via
// __IOV_MAX); some libc/kernel-header combinations don't expose it at all.
// 1024 is the long-standing Linux kernel constant (UIO_FASTIOV lineage,
// unrelated to IORING_MAX_REG_BUFFERS) enforced by import_iovec() /
// io_uring's own buffer-registration path, so fall back to it explicitly
// rather than silently skipping the bounds check below.
#ifndef UIO_MAXIOV
...
DEFINE_int32(iouring_mem_pool_initial_mb, 8,
             "Initial size of the io_uring fixed-buffer memory pool (MB). "
             "Effective only with --iouring_register_buffers=true. Capped "
             "internally at UIO_MAXIOV (1024) * --iouring_iobuf_block_size "
             "per region (one io_uring_register_buffers() call registers at "
             "most UIO_MAXIOV buffers at once); requesting more just makes "
             "the pool grow in additional --iouring_mem_pool_max_regions "
             "regions instead of failing.");

DEFINE_int32(iouring_mem_pool_increase_mb, 8,
             "Growth increment when the pool is exhausted (MB). "
             "Effective only with --iouring_register_buffers=true. Subject "
             "to the same per-region UIO_MAXIOV cap as "
             "--iouring_mem_pool_initial_mb.");
...
// ---------------------------------------------------------------------------
unsigned IouringMemPool::MaxBufIndexCapacity() {
    return static_cast<unsigned>(FLAGS_iouring_mem_pool_max_regions) *
           static_cast<unsigned>(UIO_MAXIOV);
}
...
bool IouringMemPool::AddRegion(size_t region_size_mb) {
    ...
    size_t region_size = region_size_mb * kBytesPerMB;

    // io_uring_register_buffers() (and the current re-registration path in
    // IouringEndpoint's ring-registrar callback, which re-registers every
    // block of a region in one call) build one iovec per block and pass the
    // array to the kernel in a single syscall. Like readv()/writev(), the
    // kernel caps the iovec count of a single such call at UIO_MAXIOV
    // (1024) -- see fs/io_uring.c's io_sqe_buffers_register(): it bails out
    // with -EINVAL past that many segments. That failure happens deep
    // inside the kernel with no indication of which parameter is at fault
    // ("Invalid argument"), so clamp the region size here instead of
    // letting it reach the kernel: silently capping keeps
    // --iouring_mem_pool_initial_mb / --iouring_mem_pool_increase_mb safe to
    // set to "however much memory you want" without the caller having to
    // reason about UIO_MAXIOV, at the cost of the pool growing in more,
    // smaller steps (bounded by --iouring_mem_pool_max_regions) than asked.
    const size_t max_bytes_per_region =
        static_cast<size_t>(UIO_MAXIOV) * block_size_;
    if (region_size > max_bytes_per_region) {
        LOG(WARNING)
            << "IouringMemPool: requested region size " << region_size_mb
            << " MiB / block_size=" << block_size_ << " bytes exceeds "
            << UIO_MAXIOV << " blocks (the kernel's per-call "
               "io_uring_register_buffers() limit, UIO_MAXIOV); clamped to "
            << (max_bytes_per_region / kBytesPerMB) << " MiB ("
            << UIO_MAXIOV << " blocks) for this region.";
        region_size = max_bytes_per_region;
    }
