...
public:
    ...
    // Upper bound on the number of buf_index slots this pool could ever
    // need: --iouring_mem_pool_max_regions regions, each capped at
    // UIO_MAXIOV blocks (see AddRegion's per-region clamp).  Callers use
    // this to io_uring_register_buffers_sparse() the full capacity up
    // front (once, per ring, before any region exists) so that every
    // later AddRegion() can register its blocks via
    // io_uring_register_buffers_update_tag() -- an in-place update within
    // the already-reserved table, never an unregister/register cycle that
    // would invalidate buf_index values already handed out for older
    // regions.  Pure computation from gflags; safe to call before Init().
    static unsigned MaxBufIndexCapacity();
