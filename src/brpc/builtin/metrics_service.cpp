...
public:
    explicit MetricsDumper(butil::IOBufBuilder& os,
...
bool MetricsDumper::dump(const std::string& name, const butil::StringPiece& desc) {
    if (desc.size() > 0 && desc[0] == '"') {
...
bool MetricsDumper::ProcessLatencyRecorderSuffix(const butil::StringPiece& name,
                                                 const butil::StringPiece& desc) {
    ...
    for (; i < NPERCENTILES; ++i) {
        if (name.starts_with(_server_prefix) && name.ends_with(latency_names[i])) {
