...
template <class T1, class T2, bool = base::is_empty<T2>::value>
class UniquePtrStorage {
    T1 t1_;
    T2 t2_;

    typedef typename base::add_reference<T2>::type T2_reference;
    typedef typename base::add_reference<const T2>::type T2_const_reference;
...
struct default_delete {
    ...
    template <class U>
        default_delete(const default_delete<U>&,
            typename base::enable_if<up_detail::is_convertible<U*, T*>::value>::type* = 0) {}
...
struct pointer_type {
    typedef typename pointer_type_imp::pointer_type<T,
        typename base::remove_reference<D>::type>::type type;
};
...
private:
    ...
    typedef typename base::add_reference<deleter_type>::type deleter_reference;
    typedef typename base::add_reference<const deleter_type>::type deleter_const_reference;
...
public:
    ...
    unique_ptr(pointer p, typename base::conditional<base::is_reference<D>::value,
               volatile typename base::remove_reference<D>::type&, D>::type d)
        : ptr_(move(p), forward<D>(const_cast<typename base::add_reference<D>::type>(d))) {}

    template <class U, class E>
    unique_ptr(unique_ptr<U, E> u,
               typename base::enable_if <
               !base::is_array<U>::value &&
               up_detail::is_convertible<typename unique_ptr<U>::pointer, pointer>::value
&&
               up_detail::is_convertible<E, deleter_type>::value &&
               (
                   !base::is_reference<deleter_type>::value ||
                   base::is_same<deleter_type, E>::value)
               >::type* = 0)
        : ptr_(u.release(), forward<D>(forward<E>(u.get_deleter()))) {}

    ~unique_ptr() {
        BAIDU_CASSERT(!base::is_reference<deleter_type>::value, deleter_is_ref);
        BAIDU_CASSERT(!base::is_pointer<deleter_type>::value, deleter_is_pointer);
        reset();
    }
    ...
    typename base::add_reference<T>::type operator*() const {return *get();}
...
private:
    template <class U>
    explicit unique_ptr(
        U, typename base::enable_if<up_detail::is_convertible<U, pointer>::value>::type* = 0);

    template <class U>
    unique_ptr(U, typename base::conditional<base::is_reference<D>::value,
               volatile typename base::remove_reference<D>::type&, D>::type,
               typename base::enable_if<up_detail::is_convertible<U, pointer>::value>::type* = 0);
};
