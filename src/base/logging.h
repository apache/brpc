...
#include "base/atomicops.h" // Used by LOG_EVERY_N, LOG_FIRST_N etc
...
public:
    ...
    virtual bool OnLogMessage(int severity, const char* file, int line,
                              const base::StringPiece& log_content) = 0;
private:
...
public:
    bool OnLogMessage(int severity, const char* file, int line,
                 const base::StringPiece& log_content);
private:
    base::Lock _lock;
};
...
#define BAIDU_LOG_IF_EVERY_N_IMPL(logifmacro, severity, condition, N)   \
    static ::base::subtle::Atomic32 BAIDU_CONCAT(logeveryn_, __LINE__) = -1; \
    const static int BAIDU_CONCAT(logeveryn_sc_, __LINE__) = (N);       \
    const int BAIDU_CONCAT(logeveryn_c_, __LINE__) =                    \
        ::base::subtle::NoBarrier_AtomicIncrement(&BAIDU_CONCAT(logeveryn_, __LINE__), 1); \
...
#define BAIDU_LOG_IF_FIRST_N_IMPL(logifmacro, severity, condition, N)   \
    static ::base::subtle::Atomic32 BAIDU_CONCAT(logfstn_, __LINE__) = 0; \
    logifmacro(severity, (condition) && BAIDU_CONCAT(logfstn_, __LINE__) < N && \
               ::base::subtle::NoBarrier_AtomicIncrement(&BAIDU_CONCAT(logfstn_, __LINE__), 1) <= N)

// Helper macro included by all *_EVERY_SECOND macros.
#define BAIDU_LOG_IF_EVERY_SECOND_IMPL(logifmacro, severity, condition) \
    static ::base::subtle::Atomic64 BAIDU_CONCAT(logeverys_, __LINE__) = 0; \
    const int64_t BAIDU_CONCAT(logeverys_ts_, __LINE__) = ::base::gettimeofday_us(); \
...
#define CHECK(condition)                                                \
    !(condition) ? ::base::debug::BreakDebugger() : BAIDU_EAT_STREAM_PARAMS
...
public:
    ...
    base::StringPiece content() const
...
inline void LogAtLevel(int const log_level, const base::StringPiece &msg) {
