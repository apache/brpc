...
// static
base::Mutex* LoggingLock::log_lock = NULL;
// static
...
void print_log_prefix(std::ostream& os,
    ...
    os << ' ' << std::setfill(' ') << std::setw(5)
       << base::PlatformThread::CurrentId() << std::setfill('0');
...
class DoublyBufferedLogSink : public base::DoublyBufferedData<LogSink*> {
public:
...
bool StringSink::OnLogMessage(int severity, const char* file, int line, 
    ...
    {
        base::AutoLock lock_guard(_lock);
    ...
    if (FLAGS_print_stack_on_check && _is_check && _severity == BLOG_FATAL) {
        ...
        *this << std::endl;  // Newline to separate from log message.
        if (count > 3) {
            ...
            // #2 may be ~LogStream
            //   #0 0x00000059ccae base::debug::StackTrace::StackTrace()
            //   #1 0x0000005947c7 logging::LogStream::FlushWithoutReset()
            //   #2 0x000000594b88 logging::LogMessage::~LogMessage()
            base::debug::StackTrace trace_stripped(addrs + 3, count - 3);
...
struct VLogSite {
    ...
    VLogSite* next() { return (VLogSite*)base::subtle::Acquire_Load(&_next); }
    const VLogSite* next() const
    { return (VLogSite*)base::subtle::Acquire_Load(&_next); }
    void set_next(VLogSite* next)
    { base::subtle::Release_Store(&_next, (base::subtle::AtomicWord)next); }
...
struct VModuleList {
    ...
    int init(const char* vmodules) {
        ...
        for (base::StringSplitter sp(vmodules, ','); sp; ++sp) {
    ...
    if (old_module_list) {
        ...
        deleting_vmodule_list->push_back(
            std::make_pair(old_module_list,
                           base::gettimeofday_us() + DELAY_DELETION_SEC * 1000000L));
        while (!deleting_vmodule_list->empty() &&
               deleting_vmodule_list->front().second <= base::gettimeofday_us()) {
