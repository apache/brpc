...
public:
    ...
    struct BlockGroup {
        ...
        base::atomic<Block*> blocks[RP_GROUP_NBLOCK];
        ...
        BlockGroup() : nblock(0) {
            ...
            memset(blocks, 0, sizeof(base::atomic<Block*>) * RP_GROUP_NBLOCK);
        }
    };
...
#undef BAIDU_RESOURCE_POOL_GET
    ...
    static inline T* unsafe_address_resource(ResourceId<T> id) {
        ...
        return (T*)(_block_groups[(block_index >> RP_GROUP_NBLOCK_NBIT)]
                    .load(base::memory_order_consume)
                    ->blocks[(block_index & (RP_GROUP_NBLOCK - 1))]
                    .load(base::memory_order_consume)->items) +
    ...
    static inline T* address_resource(ResourceId<T> id) {
        ...
        if (__builtin_expect(group_index < RP_MAX_BLOCK_NGROUP, 1)) {
            BlockGroup* bg =
                _block_groups[group_index].load(base::memory_order_consume);
    ...
    static inline ResourcePool* singleton() {
        ResourcePool* p = _singleton.load(base::memory_order_consume);
        if (p) {
            return p;
        }
...
private:
    ...
    static base::static_atomic<ResourcePool*> _singleton;
...
template <typename T>
base::static_atomic<ResourcePool<T>*> ResourcePool<T>::_singleton =
...
template <typename T>
base::static_atomic<long> ResourcePool<T>::_nlocal = BASE_STATIC_ATOMIC_INIT(0);

template <typename T>
base::static_atomic<size_t> ResourcePool<T>::_ngroup = BASE_STATIC_ATOMIC_INIT(0);
...
template <typename T>
base::static_atomic<typename ResourcePool<T>::BlockGroup*>
ResourcePool<T>::_block_groups[RP_MAX_BLOCK_NGROUP] = {};

#ifdef BASE_RESOURCE_POOL_NEED_FREE_ITEM_NUM
template <typename T>
base::static_atomic<size_t> ResourcePool<T>::_global_nfree = BASE_STATIC_ATOMIC_INIT(0);
#endif
