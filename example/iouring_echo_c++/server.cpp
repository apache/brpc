...
//   --iouring_max_inflight_writes=N Per-socket cap on in-flight WRITE/
//                                   WRITE_FIXED SQEs before IsWritable()
//                                   returns false and writes fall back to a
//                                   synchronous fd write (default 64).
//
...
//   --iouring_register_buffers      Enable READ_FIXED / WRITE_FIXED. Each
//                                   Poller ring reserves a sparse buf_index
//                                   table (io_uring_register_buffers_sparse,
//                                   requires kernel >= 5.19) sized for
//                                   iouring_mem_pool_max_regions * UIO_MAXIOV
//                                   slots up front, then fills it
//                                   incrementally via
//                                   io_uring_register_buffers_update_tag() as
//                                   the pool grows -- older regions' buffers
//                                   stay valid across growth. If the kernel
//                                   doesn't support sparse registration, the
//                                   process fails fast at startup.
//   --iouring_mem_pool_initial_mb=N Initial registered memory (default 8
//                                   MiB). Per-region size is capped at
//                                   UIO_MAXIOV (1024) * iobuf_block_size --
//                                   a single buffer-registration call cannot
//                                   cover more than 1024 buffers, so larger
//                                   requests are silently clamped per region
//                                   (see iouring_mem_pool_max_regions to grow
//                                   the pool beyond one region).
//   --iouring_mem_pool_increase_mb=N Growth step (default 8 MiB, same
//                                    per-region UIO_MAXIOV cap as above).
//   --iouring_mem_pool_max_regions=N Max growth regions (default 8); also
//                                    determines the sparse table capacity
//                                    reserved at startup (see above).
//   --iouring_iobuf_block_size=N    IOBuf block / slot size in bytes (default 8192).
//   --iouring_mem_pool_free_buckets=N  Free-list buckets, reduces mutex
//                                      contention under high concurrency
//                                      (default 8).
//   --iouring_mem_pool_tls_cache_num=N Per-thread cached-block limit in the
//                                      TLS free list (default 128).
// ---------------------------------------------------------------------------
