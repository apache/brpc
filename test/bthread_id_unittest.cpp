...
TEST(BthreadIdTest, join_after_destroy) {
    bthread_id_t id1;
    int x = 0xdead;
    ASSERT_EQ(0, bthread_id_create_ranged(&id1, &x, nullptr, 2));
...
TEST(BthreadIdTest, join_before_destroy) {
    bthread_id_t id1;
    int x = 0xdead;
    ASSERT_EQ(0, bthread_id_create(&id1, &x, nullptr));
    ASSERT_EQ(get_version(id1), bthread::id_value(id1));
    pthread_t th[8];
    SignalArg args[ARRAY_SIZE(th)];
    for (size_t i = 0; i < ARRAY_SIZE(th); ++i) {
...
TEST(BthreadIdTest, default_error_is_destroy) {
    bthread_id_t id1;
    ASSERT_EQ(0, bthread_id_create(&id1, nullptr, nullptr));
    ASSERT_EQ(get_version(id1), bthread::id_value(id1));
    ASSERT_EQ(0, bthread_id_error(id1, EBADF));
    ASSERT_EQ(get_version(id1) + 4, bthread::id_value(id1));
}
...
TEST(BthreadIdTest, doubly_destroy) {
    bthread_id_t id1;
    ASSERT_EQ(0, bthread_id_create_ranged(&id1, nullptr, nullptr, 2));
    bthread_id_t id2 = { id1.value + 1 };
    ASSERT_EQ(get_version(id1), bthread::id_value(id1));
    ASSERT_EQ(get_version(id1), bthread::id_value(id2));
    ASSERT_EQ(0, bthread_id_error(id1, EBADF));
...
TEST(BthreadIdTest, many_error) {
    ...
    ASSERT_EQ(0, bthread_id_trylock(id1, nullptr));
...
static void* locker(void* arg) {
    ...
    butil::Timer tm;
    tm.start();
    EXPECT_EQ(0, bthread_id_lock(id, nullptr));
    bthread_usleep(2000);
    EXPECT_EQ(0, bthread_id_unlock(id));
    tm.stop();
...
static void* failed_locker(void* arg) {
    bthread_id_t id = { (uintptr_t)arg };
    int rc = bthread_id_lock(id, nullptr);
    if (rc == 0) {
        bthread_usleep(2000);
...
TEST(BthreadIdTest, stop_a_wait_after_fight_before_signal) {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(th); ++i) {
        ...
        ASSERT_EQ(0, bthread_start_urgent(&th[i], nullptr, stopped_waiter, &args[i]));
    }
...
TEST(BthreadIdTest, reset_range) {
    bthread_id_t id;
    ASSERT_EQ(0, bthread_id_create(&id, nullptr, nullptr));
    ASSERT_EQ(0, bthread_id_lock_and_reset_range(id, nullptr, 1000));
    bthread::id_status(id, std::cout);
    bthread_id_unlock(id);
    ASSERT_EQ(0, bthread_id_lock_and_reset_range(id, nullptr, 300));
    bthread::id_status(id, std::cout);
    bthread_id_unlock_and_destroy(id);
}
...
static void* fail_to_lock_id(void* args_in) {
    ...
    butil::Timer tm;
    EXPECT_EQ(args->expected_return, bthread_id_lock(args->id, nullptr));
...
TEST(BthreadIdTest, about_to_destroy_before_locking) {
    bthread_id_t id;
    ASSERT_EQ(0, bthread_id_create(&id, nullptr, nullptr));
    ASSERT_EQ(0, bthread_id_lock(id, nullptr));
    ...
    ASSERT_EQ(0, bthread_start_background(&bth, nullptr, fail_to_lock_id, &args));
...
TEST(BthreadIdTest, about_to_destroy_cancelled) {
    ...
    ASSERT_EQ(0, bthread_start_background(&bth, nullptr, succeed_to_lock_id, &id));
    // The threads should quit soon.
    pthread_join(pth, nullptr);
    bthread_join(bth, nullptr);
    bthread::id_status(id, std::cout);
...
TEST(BthreadIdTest, error_with_descriptions) {
    ...
    ASSERT_EQ(0, bthread_id_lock(id1, nullptr));
    ASSERT_EQ(0, bthread_id_error(id1, expected_code));
    ASSERT_EQ(0, bthread_id_unlock(id1));
    ASSERT_EQ(branch_counter, branch_tags[1]);
    ...
    ASSERT_EQ(0, bthread_id_lock(id2, nullptr));
    ASSERT_EQ(0, bthread_id_error2(id2, expected_code, expected_desc));
