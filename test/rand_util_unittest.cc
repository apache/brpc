...
TEST(RandUtilTest, Sanity) {
    ...
    for (int i = 0; i < 10; ++i) {
        uint64_t value = base::fast_rand_in(
    ...
    for (int i = 0; i < 10; ++i) {
        int64_t value = base::fast_rand_in(
    ...
    EXPECT_EQ(base::fast_rand_in(-1, -1), -1);
    EXPECT_EQ(base::fast_rand_in(1, 1), 1);
    EXPECT_EQ(base::fast_rand_in(0, 0), 0);
    EXPECT_EQ(base::fast_rand_in(std::numeric_limits<int64_t>::min(),
                                  std::numeric_limits<int64_t>::min()),
              std::numeric_limits<int64_t>::min());
    EXPECT_EQ(base::fast_rand_in(std::numeric_limits<int64_t>::max(),
                                  std::numeric_limits<int64_t>::max()),
              std::numeric_limits<int64_t>::max());
    EXPECT_EQ(base::fast_rand_in(std::numeric_limits<uint64_t>::min(),
                                  std::numeric_limits<uint64_t>::min()),
              std::numeric_limits<uint64_t>::min());
    EXPECT_EQ(base::fast_rand_in(std::numeric_limits<uint64_t>::max(),
...
TEST(RandUtilTest, RandDouble) {
    ...
    volatile double number2 = base::fast_rand_double();
...
TEST(RandUtilTest, RandGeneratorIsUniform) {
    ...
    for (int round = 0; round < 2; ++round) {
        ...
        while (count < kMaxAttempts) {
            uint64_t value = (round == 0 ? base::RandGenerator(kTopOfRange)
                              : base::fast_rand_less_than(kTopOfRange));
...
TEST(RandUtilTest, RandUint64ProducesBothValuesOfAllBits) {
    ...
    for (int round = 0; round < 2; ++round) {
        ...
        for (size_t i = 0; i < 1000; ++i) {
            uint64_t value = (round == 0 ? base::RandUint64() : base::fast_rand());
            found_ones |= value;
            found_zeros &= value;
...
TEST(RandUtilTest, fast_rand_perf) {
    ...
    tm.start();
    for (int i = 0; i < kTestIterations; ++i) {
        s += base::fast_rand_less_than(kRange);
    }
    tm.stop();
