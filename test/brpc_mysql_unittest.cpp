...
static void CheckMysqlServer() {
    brpc::MYSQL_table_suffix = butil::gettimeofday_us();
...
TEST_F(MysqlTest, ok) {
    ...
    ASSERT_EQ(0, channel.Init(ss.str().c_str(), &options));
    {
        ...
        brpc::Controller cntl;
        std::stringstream ss;
        ss << "drop table brpc_table_" << brpc::MYSQL_table_suffix;
    ...
    }
    {
        ...
        brpc::Controller cntl;
        std::stringstream ss;
        ss << "CREATE TABLE IF NOT EXISTS `brpc_table_" << brpc::MYSQL_table_suffix
           << "` (`col1` int(11) NOT NULL AUTO_INCREMENT, "
              "`col2` varchar(45) DEFAULT NULL, "
              "`col3` decimal(6,3) DEFAULT NULL, `col4` datetime DEFAULT NULL, `col5` blob, `col6` "
              "binary(6) DEFAULT NULL, `col7` tinyblob, `col8` longblob, `col9` mediumblob, "
              "`col10` "
              ...
              "DEFAULT NULL, `col21` polygon DEFAULT NULL, `col22` bigint(64) DEFAULT NULL, "
              "`col23` "
              "decimal(10,0) DEFAULT NULL, `col24` double DEFAULT NULL, `col25` float DEFAULT "
              "NULL, "
              ...
              "`col34` mediumtext, `col35` tinytext, `col36` tinytext, `col37` bit(7) DEFAULT "
              "NULL, "
...

    {
        ...
        ss1 << "INSERT INTO `brpc_table_" << brpc::MYSQL_table_suffix
            << "` "
               ...
               "14:12:09.019473' ,'01:06:09','1970-12-08 00:00:00.0001' "
...
TEST_F(MysqlTest, resultset) {
...

    {
        ...
        brpc::Controller cntl;
        std::stringstream ss;
        ss << "select count(0) from brpc_table_" << brpc::MYSQL_table_suffix;
        request.Query(ss.str());
        channel.CallMethod(NULL, &cntl, &request, &response, NULL);
        ASSERT_FALSE(cntl.Failed()) << cntl.ErrorText();
        // ASSERT_EQ(1ul, response.reply_size());
...

    {
        ...
        brpc::Controller cntl;
        std::stringstream ss;
        ss << "select * from brpc_table_" << brpc::MYSQL_table_suffix << " where 1 = 2";
        request.Query(ss.str());
        channel.CallMethod(NULL, &cntl, &request, &response, NULL);
        ASSERT_FALSE(cntl.Failed()) << cntl.ErrorText();
        ASSERT_EQ(1ul, response.reply_size());
...

    {
        ...
        for (uint64_t idx = 0; idx < reply.row_count(); ++idx) {
            ...
            ASSERT_EQ(row.field(14).string(), "1970-12-08 00:00:00.0001");
...
TEST_F(MysqlTest, transaction) {
...

    {
        ...
        brpc::Controller cntl;
        std::stringstream ss;
        ss << "drop table brpc_tx_" << brpc::MYSQL_table_suffix;
        request.Query(ss.str());
        channel.CallMethod(NULL, &cntl, &request, &response, NULL);
    }
    {
        ...
        brpc::Controller cntl;
        std::stringstream ss;
        ss << "CREATE TABLE IF NOT EXISTS `brpc_tx_" << brpc::MYSQL_table_suffix
           << "` (`Id` int(11) NOT NULL AUTO_INCREMENT,`LastName` "
              "varchar(255) DEFAULT "
    ...
    }
    {
        ...
        uint64_t idx1, idx2;
        {
            ...
            std::stringstream ss;
            ss << "insert into brpc_tx_" << brpc::MYSQL_table_suffix
               << "(LastName,FirstName, Address) values "
                  "('lucy',12.5,'beijing')";
            ASSERT_EQ(request.Query(ss.str()), true);
            brpc::MysqlResponse response;
            brpc::Controller cntl;
            channel.CallMethod(NULL, &cntl, &request, &response, NULL);
            ASSERT_FALSE(cntl.Failed()) << cntl.ErrorText();
        ...
        // not commit, so return 0 rows
        {
            ...
            brpc::Controller cntl;
            std::stringstream ss;
            ss << "select * from brpc_tx_" << brpc::MYSQL_table_suffix << " where id in (" << idx1
               << "," << idx2 << ")";
            request.Query(ss.str());
            channel.CallMethod(NULL, &cntl, &request, &response, NULL);
            ASSERT_FALSE(cntl.Failed()) << cntl.ErrorText();
            ASSERT_EQ(1ul, response.reply_size());
            ASSERT_EQ(response.reply(0).row_count(), 0ul);
        }
        ...
        // after commit, so return 2 rows
        {
            ...
            ASSERT_FALSE(cntl.Failed()) << cntl.ErrorText();
            ASSERT_EQ(1ul, response.reply_size());
            ASSERT_EQ(response.reply(0).row_count(), 2ul);
        }
    }

    {
...

        {
            brpc::MysqlRequest request(tx.get());
            std::stringstream ss;
            ss << "update brpc_tx_" << brpc::MYSQL_table_suffix
               << " set Address = 'hangzhou' where Id=1";
            ASSERT_EQ(request.Query(ss.str()), true);
            brpc::MysqlResponse response;
            brpc::Controller cntl;
            channel.CallMethod(NULL, &cntl, &request, &response, NULL);
...
TEST_F(MysqlTest, statement) {
    ...
    {
        std::stringstream ss;
        ss << "select * from brpc_table_" << brpc::MYSQL_table_suffix << " limit 1";
        auto stmt(brpc::NewMysqlStatement(channel, ss.str()));
        ASSERT_FALSE(stmt == NULL) << "Fail to create statement";
        {
            ...
            ASSERT_FALSE(cntl.Failed()) << cntl.ErrorText();
            ASSERT_EQ(1ul, response.reply_size());
            ASSERT_EQ(response.reply(0).is_resultset(), true);
        }
        {
            brpc::MysqlRequest request(stmt.get());
            ASSERT_EQ(request.AddParam(1157), true);
    ...
    {
        std::stringstream ss;
        ss << "select * from brpc_table_" << brpc::MYSQL_table_suffix << " where col1 = ?";
        auto stmt(brpc::NewMysqlStatement(channel, ss.str()));
        ASSERT_FALSE(stmt == NULL) << "Fail to create statement";
        {
            brpc::MysqlRequest request(stmt.get());
        ...
        }
        {
            ...
            ASSERT_FALSE(cntl.Failed()) << cntl.ErrorText();
            ASSERT_EQ(1ul, response.reply_size());
            ASSERT_EQ(response.reply(0).is_error(), true);
        }
    }
    // two parameter
    {
        std::stringstream ss;
        ss << "select * from brpc_table_" << brpc::MYSQL_table_suffix
           << " where col1 = ? and col2 = ?";
        auto stmt(brpc::NewMysqlStatement(channel, ss.str()));
        ASSERT_FALSE(stmt == NULL) << "Fail to create statement";
        {
            brpc::MysqlRequest request(stmt.get());
            ASSERT_EQ(request.AddParam(1157), true);
            ASSERT_EQ(request.AddParam("col2"), true);
            brpc::MysqlResponse response;
            brpc::Controller cntl;
            channel.CallMethod(NULL, &cntl, &request, &response, NULL);
            ASSERT_FALSE(cntl.Failed()) << cntl.ErrorText();
...
TEST_F(MysqlTest, drop_table) {
    ...
    ASSERT_EQ(0, channel.Init(ss.str().c_str(), &options));
    {
        ...
        brpc::Controller cntl;
        std::stringstream ss;
        ss << "delete from brpc_table_" << brpc::MYSQL_table_suffix;
        request.Query(ss.str());
        channel.CallMethod(NULL, &cntl, &request, &response, NULL);
        ASSERT_FALSE(cntl.Failed()) << cntl.ErrorText();
        ASSERT_EQ(1ul, response.reply_size());
